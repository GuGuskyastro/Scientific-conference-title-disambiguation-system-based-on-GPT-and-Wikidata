...
# Create query and comparison tool to determine whether the extracted conference has a record in the weaviate VS
def weaviateQuery(title):

    # Query similar entities for the entered conferene title
    response = (
        client.query
    ...
    # Send the extraction results and query results to LLM and give a judgment
    query_Input_prompt = PromptTemplate(
        ...
        If yes, then answer: conference title: xxx(database query result) |  corresponding QID 
        If no, then answer: conference title: xxx , This conference is not in the database!

         Two text:{text_input}'''
    )
    weaviateQuery_chain = LLMChain(llm=llm, prompt=query_Input_prompt)
    return weaviateQuery_chain.run(merged)


# Define the above methods as tools used in Agent
tools = [
    Tool(
        name="Extraction",
        func=extration,
    ...
    Tool(
        name="Query",
        func=weaviateQuery,
        description="Supports one conference title input at a time. Used for querying conference information from the database,and determine whether the query meeting is stored in the database"
    )
]
...
2. Use the Query tool to query conference title one by one, that is, one of the conference title returned by Extraction tool is used as the input of query tool. Never give short names as input to this tool! Only query titles parsed from user input, never invent new ones yourself! Repeat this step until all conference in step 1 result are judged. 

Now you dont need use tools any more. Summarize the results of the judgment and give the final answer like:
There are a total of x citations in the input text(which is user input), of which y contains scientific conference information, and z scientific conference information is also in the database. 
